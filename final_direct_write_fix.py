...
def apply_final_direct_write_fix():
    """
    Apply a final fix to the run_backtest function using the direct file writing approach
    that we've confirmed works correctly.
    """
    ...
    if run_backtest_start == -1 or next_def == -1:
    ...
    # Create a modified version that uses direct file writing
    modified_run_backtest = """def run_backtest(start_date, end_date, mode='backtest', initial_capital=10000, 
...
"""
    ...
    # Replace the original run_backtest function with the modified version
    new_content = content[:run_backtest_start] + modified_run_backtest + content[next_def:]
    
    # Write the updated content back to the file
    with open(file_path, 'w') as file:
        file.write(new_content)
    
    print("Successfully applied the final direct write fix to the run_backtest function")
