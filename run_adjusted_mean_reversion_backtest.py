...
"""
Adjusted Mean Reversion Strategy Backtest
-----------------------------------------
This script runs a backtest for the adjusted mean reversion strategy using real Alpaca data.
It uses more relaxed parameters to generate more trading signals.
"""
...
from mean_reversion_strategy_adjusted import MeanReversionStrategyAdjusted
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("adjusted_mean_reversion_backtest.log"),
...
def run_backtest(api, symbols, start_date, end_date, config):
    ...
    # Initialize strategy with adjusted parameters
    strategy = MeanReversionStrategyAdjusted(config)
...
def save_summary(metrics, filename, start_date, end_date, symbols):
    ...
    with open(filename, 'w') as f:
        f.write(f"Adjusted Backtest Summary: {start_date} to {end_date}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run backtest for adjusted mean reversion strategy with Alpaca data')
    ...
    if not args.symbols:
        args.symbols = ['AAPL', 'MSFT', 'AMZN', 'GOOGL', 'META', 'NVDA', 'AMD', 'TSLA']
    ...
    if metrics:
        # Plot equity curve
        plot_title = f"Adjusted Strategy Equity Curve: {args.start} to {args.end}"
        plot_filename = f"adjusted_equity_curve_{args.start}_to_{args.end}.png"
        plot_equity_curve(metrics['equity_curve'], plot_title, plot_filename)
        
        # Save trade results
        trades_filename = f"adjusted_trades_{args.start}_to_{args.end}.csv"
        save_trade_results(metrics['trades'], trades_filename)
        
        # Save summary
        summary_filename = f"adjusted_summary_{args.start}_to_{args.end}.txt"
        save_summary(metrics, summary_filename, args.start, args.end, args.symbols)
        
        # Print summary
        print(f"\nAdjusted Backtest Summary: {args.start} to {args.end}")
