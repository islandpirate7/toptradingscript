...
from final_sp500_strategy_no_seasonality import run_backtest  # Import from no-seasonality version
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join('logs', f"backtest_no_seasonality_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")),
...
def display_performance_metrics(summary):
    ...
    print("\n===== PERFORMANCE METRICS (NO SEASONALITY) =====")
...
def run_quarter_backtest(quarter, start_date, end_date, max_signals, initial_capital, weekly_selection=False):
    ...
    try:
        print(f"\n==================================================")
        print(f"Running backtest WITHOUT seasonality for {quarter}: {start_date} to {end_date}")
        print(f"==================================================")
...
def run_multiple_backtests(quarter, start_date, end_date, max_signals=100, initial_capital=300, num_runs=5, random_seed=42, continuous_capital=False, previous_capital=None, weekly_selection=False):
    ...
    try:
        print(f"\n==================================================")
        print(f"Running {num_runs} backtests WITHOUT seasonality for {quarter}: {start_date} to {end_date}")
        print(f"==================================================")
        ...
        print("\n===== AVERAGED PERFORMANCE METRICS (NO SEASONALITY) =====")
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description='Run comprehensive backtest WITHOUT seasonality for specified quarters')
        ...
        if 'all' in args.quarters:
            print("Running backtest for all quarters WITHOUT seasonality")
