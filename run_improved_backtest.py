...
"""
Run Improved Combined Strategy Backtest
---------------------------------------
This script runs a backtest for the improved combined strategy
using custom technical indicators instead of TA-Lib.
"""
...
from backtest_combined_strategy_no_talib import Backtester, BacktestResults

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename='improved_combined_strategy_backtest.log',
    filemode='w'
)
...
def run_backtest(config_file='configuration_combined_strategy.yaml', 
    ...
    # Save report to file
    report_file = f"improved_combined_strategy_report_{start_date}_to_{end_date}.md"
    ...
    plt.title(f"Improved Combined Strategy Equity Curve ({start_date} to {end_date})")
    plt.xlabel("Date")
    plt.ylabel("Equity ($)")
    plt.grid(True)
    equity_curve_file = f"improved_combined_strategy_equity_{start_date}_to_{end_date}.png"
    ...
    if not trades_df.empty:
        trades_file = f"improved_combined_strategy_trades_{start_date}_to_{end_date}.csv"
        trades_df.to_csv(trades_file, index=False)
        logger.info(f"Saved trades to {trades_file}")
    
    # Save metrics to JSON
    metrics_file = f"improved_combined_strategy_metrics_{start_date}_to_{end_date}.json"
...
def compare_quarterly_results(results):
    ...
    comparison_df.to_csv("improved_combined_strategy_quarterly_comparison.csv")
    ...
    plt.tight_layout()
    plt.savefig("improved_combined_strategy_quarterly_comparison.png")
    plt.close()
