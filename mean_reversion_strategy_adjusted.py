...
"""
Adjusted Mean Reversion Strategy Implementation
-------------------------------------
This module implements a slightly adjusted version of the mean reversion strategy
with more relaxed parameters to generate more trading signals.
"""
...
class MeanReversionStrategyAdjusted:
    """Mean reversion trading strategy using Bollinger Bands and RSI with adjusted parameters"""
    
    def __init__(self, config):
        """Initialize with parameters from config, but with some adjustments for more signals"""
        mr_config = config.get('strategy_configs', {}).get('MeanReversion', {})
        
        # Load parameters from config but adjust some for more signals
        self.bb_period = mr_config.get('bb_period', 20)
        self.bb_std = 1.5  # Reduced from 1.9 to catch more mean reversion opportunities
        self.rsi_period = mr_config.get('rsi_period', 14)
        self.rsi_overbought = 70  # Increased from 65
        self.rsi_oversold = 30    # Decreased from 35
        self.require_reversal = False  # Don't require price reversal for more signals
        ...
        logger.info(f"Initialized ADJUSTED strategy with parameters: BB period={self.bb_period}, BB std={self.bb_std}, "
